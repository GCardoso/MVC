...
class  MoobiDatabaseHandler  {
    ...
    public function __construct($aConfigJson) {
    	...
    	if(!empty($aConfigJson['usuario'])) {
    		self::$sUser = $aConfigJson['usuario'];
    	}
	...
	public static function getNewInstance() {	
    	...
    	public static function execute ($sSql,$aDados,$rConexao) {
    		$resultado = $rConexao->prepare($sSql)->execute($aDados);
            return $resultado;
    	}
        ...
        public static function executeAux ($sSql,$rConexao) {
            $resultado = $rConexao->prepare($sSql)->execute();
            return $resultado;
        }

        public static function getQuantidadeDePaginas ($rQuery,$iDivisor) {
            $iResultado = $rQuery->rowCount()/$iDivisor;
            $iResultado = round($iResultado);
            return $iResultado;
        ...
        public static function getDatabase() {
             ...
             $rDatabase = new MoobiDatabaseHandler($rJson->getJson());
             return $rDatabase;
        }

        public static function consultarTodos($sSql, $rConexao) {
            $aResultado = $rConexao->query($sSql);
            return $aResultado->fetchAll();
        }
}