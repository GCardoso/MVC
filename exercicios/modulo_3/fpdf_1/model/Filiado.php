...
class Filiado {
	...
	public static function listarTodosFiliados ($aDados = []) {

  		if (empty($aDados['orderby'])) {
  			$aDados['orderby'] = 'COUNT(d.id_dependente) DESC';
  		}

		$rDatabase = MoobiDatabaseHandler::getDatabase();
		$rConexao = $rDatabase->getInstance();
		$sSql = "SELECT f.nome , f.cpf , f.rg ,f.data_de_nascimento,COUNT(d.id_dependente) Dependentes , f.id_empresa 
		         FROM av1_filiados f left JOIN
  		         av1_dependentes d ON f.id_filiado = d.id_filiado
		         GROUP BY
  		         f.nome,f.cpf,f.rg,f.data_de_nascimento , f.id_empresa
  		         ORDER BY ".$aDados['orderby'];

  		$aConsulta = $rConexao->query($sSql);
  		return $aConsulta->fetchAll();         
	}
