...
class Empresa {
	...
	public static function buscaUnica ($aId_empresa) {
		...
		$rResultado = $rConexao->query($sSql);
		return $rResultado->fetch();
	...
	public static function listarEmpresaQtdFiliados () {

		$rDatabase = MoobiDatabaseHandler::getDatabase();
		$rConexao = $rDatabase->getInstance();
		$sSql = "SELECT e.nome,e.id_empresa, count(f.id_filiado)
				 FROM av1_empresa e left join av1_filiados f 
				 on e.id_empresa = f.id_empresa  group by 
				 e.id_empresa,e.nome order by e.nome";
		$rResultado = $rConexao->query($sSql);
		return $rResultado->fetchAll();	
